...
def parse_input(
    inp: List[str], session: Session, order: Order, sender: str, members: Dict[str, str]
) -> (Order, str):
    def add(namespace: Dict[str, Any]) -> (Order, str):
        ...
        return (
            order_to_return,
            msg
            + f"Order added for {name.title()}, order: {meal_name}, price: {cent_to_euro(price)}",
        )
    ...
    def edit(namespace: Dict[str, Any]) -> (Order, str):
        ...
        if order.edit_pos(user=name, item=meal_name, amount=price):
            return (
                order,
                f"Order edited for {name.title()}, order: {meal_name}, price: {cent_to_euro(price)}",
            )
        else:
    ...
    def start(namespace: Dict[str, Any]) -> (Order, str):
        if order is None:
            if namespace["name"] is None or namespace["name"] == []:
                ...
                return Order(
                    " ".join(namespace["name"])
                ), "Started new collective order with Name: " + " ".join(
                    namespace["name"]
                )
        else:
    ...
    def remove(namespace: Dict[str, Any]) -> (Order, str):
        ...
        if remove_all:
            ...
            return (
                order,
                f"Removed order {order_to_remove} for {name.title()} from order",
            )
    ...
    def pay(namespace: Dict[str, Any]) -> (Order, str):
        ...
        if namespace["name"] is None:
            ...
            p = find_match_in_database(
                " ".join(namespace["name"]), session, active=True
            )
            if not p:
                return order, "Register user first with !ob register <name>"
            name = p.name
    ...
    def reorder(namespace: Dict[str, Any]) -> (Order, str):
        ...
        if not namespace["name"]:
            ...
            cur_user = find_match_in_database(
                " ".join(namespace["name"]).lower(), session
            )
        if cur_user is None:
        ...
        last_order = (
            session.query(DB_Order, Cuts, Participant)
            .join(Cuts, Cuts.oid == DB_Order.oid)
            .join(Participant, Cuts.pid == Participant.pid)
            .filter(Participant.name == cur_user.name)
            .filter(and_(Cuts.name != paid_string, Cuts.name != "tip"))
        ...
        if debt != 0:
            if debt > 0:
                all_user = (
                    ...
                    .where(and_(Participant.user_total < 0, Participant.is_active))
                    .order_by(Participant.user_total.asc())
                    .all()
                )
                if not all_user:
                ...
                if namespace["accept"]:
                    for diff in diffs:
                        ...
                        session.execute(
                            update(Participant)
                            .where(Participant.pid == user.pid)
                            .values(user_total=user_bal - user_diff)
                        )
                        cut = Cuts(
                            pid=user.pid,
                            cut=-user_diff,
                            name="payout from " + str(cur_user.pid),
                        )
                        session.add(cut)

                    cut = Cuts(
                        pid=cur_user.pid, cut=-cur_user.user_total, name="payout"
                    )
                    session.add(cut)
                    session.execute(
                        update(Participant)
                        .where(Participant.pid == cur_user.pid)
                        .values(user_total=0)
                    )
                    session.commit()
                    return order, f"{name.title()} receives from <=\n" + "\n".join(
                        f"{item[0].name.title():<{max_name_len}}: {cent_to_euro(- item[1])}"
                        for item in diffs
                    )
                else:
                    return (
                        order,
                        f"possible transactions:\n{name.title()} receives from <=\n"
                        + "\n".join(
                            f"{item[0].name.title():<{max_name_len}}: {cent_to_euro(- item[1])}"
                            for item in diffs
                        ),
                ...
                all_user = (
                    session.query(Participant)
                    .where(and_(Participant.user_total > 0, Participant.is_active))
                    .order_by(Participant.user_total.desc())
                    .all()
                )
                if not all_user:
    ...
    def suggest(*_) -> (Order, str):
        last_orders = (
            ...
            .filter(Participant.matrix_address == sender.lower())
            .filter(Cuts.pid == Participant.pid)
            .filter(and_((Cuts.name != paid_string), (Cuts.name != "tip")))
            .order_by(Cuts.timestamp.desc())
            .limit(5)
            .all()
        )
        return (
            order,
            f"As your last {len(last_orders)} order(s), you ordered: \n"
            + "\n".join(
    ...
    def add_money(namespace: Dict[str, Any]) -> (Order, str):
        ...
        session.add(Cuts(pid=cur_user.pid, cut=bal, name="manual"))
        session.commit()
    ...
    def balance(*_) -> (Order, str):
        all_balance = (
            session.query(
                Participant.name, Participant.matrix_address, Participant.user_total
            )
        ...
        max_address = max(
            [len(user.matrix_address) for user in all_balance if user.matrix_address]
            + [len("None")]
        )
    ...
    def join(namespace: Dict[str, Any]) -> (Order, str):
        ...
        duplicates = [
            item
            for item, count in collections.Counter(to_add.values()).items()
            if count > 1
        ]
        if duplicates:
        ...
        for user in to_add:
            ...
            if user not in users and members[user].lower() not in user_names:
                ...
                session.add(
                    Participant(matrix_address=user.lower(), name=members[user].lower())
                )
                ...
                taken_username = (
                    session.query(Participant)
                    .where(Participant.name == members[user].lower())
                    .first()
                )
                ...
                potential_user = (
                    ...
                    .where(
                        and_(
                            Participant.name == members[user].lower(),
                            Participant.matrix_address == user.lower(),
                        )
                    )
                    .first()
                )
        ...
        if not added_users:
            ...
            ret = "\n".join(
                [f"added {user}:{members[user].title()}" for user in added_users]
            ) + ("\nduplicate Usernames: " + duplicates_str if duplicates else "")
        return order, ret
    ...
    def register(namespace: Dict[str, Any]) -> (Order, str):
        name = " ".join(namespace["name"]).lower()
        address_user = (
            session.query(Participant).where(Participant.name == name).first()
        )
    ...
    def init(namespace: Dict[str, Any]) -> (Order, str):
        ...
        if cur_user.user_total == 0:
            session.add(Cuts(pid=cur_user.pid, cut=bal, name="initial balance"))
            cur_user.user_total = bal
            session.commit()
            return (
                order,
                f"Set init balance for {cur_user.name.title()} to {cent_to_euro(bal)}",
            )
        elif namespace["force"]:
            change = bal - cur_user.user_total
            cur_user.user_total = bal
            session.add(Cuts(pid=cur_user.pid, cut=change, name="initial balance"))
            session.commit()
            return (
                order,
                f"Set balance for {cur_user.name.title()} to {cent_to_euro(bal)} -- by FORCE",
            )
        else:
            return (
                order,
                f"Balance of {cur_user.name.title()} is not zero, use 'balance' to check",
            )
    ...
    def deinit(namespace: Dict[str, Any]) -> (Order, str):
        ...
        if namespace["self"]:
            cur_user = (
                session.query(Participant)
                .where(Participant.matrix_address == sender.lower())
                .first()
            )
        else:
            name = " ".join(namespace["name"]).lower()
        ...
        if cur_user is None:
            ...
            return (
                order,
                f"Balance of {cur_user.name.title()} is {cent_to_euro(cur_user.user_total)}",
            )
        elif cur_user.user_total == 0:
    ...
    def transfer(namespace: Dict[str, Any]) -> (Order, str):
        ...
        destination_user = find_match_in_database(
            " ".join(namespace["destination"]).lower(), session, True
        )
        ...
        if destination_user is None:
            return (
                order,
                f"User {namespace['destination'].title()} not registered or inactive",
            )
        if namespace["amount"]:
        ...
        session.add(
            Cuts(
                pid=origin_user.pid,
                cut=amount,
                name="transfer to " + destination_user.name,
            )
        )
        session.add(
            Cuts(
                pid=destination_user.pid,
                cut=-amount,
                name="transfer from " + origin_user.name,
            )
        )
        session.commit()
        return (
            order,
            f"Transferred {cent_to_euro(amount)} from {origin_user.name.title()} to {destination_user.name.title()}",
        )

    main_parser = argparse.ArgumentParser(
        prog="OrderBot", add_help=False, usage="%(prog)s options:"
    )
    main_subparser = main_parser.add_subparsers()

    order_parser = argparse.ArgumentParser(
        prog="UserBot", add_help=False, usage="%(prog)s options:"
    )
    order_subparser = order_parser.add_subparsers()
    order_parser = main_subparser.add_parser(
        "order", parents=[order_parser], help="manages orders"
    )
    ...
    user_parser = main_subparser.add_parser(
        "user", parents=[user_parser], help="manages users"
    )
    user_parser.add_argument("user", action="store_true", help="manages users")

    start_parser = order_subparser.add_parser(
        cmd[4], help="start a new collective order"
    )
    start_parser.set_defaults(func=start)
    start_parser.add_argument(
        "name",
        nargs=argparse.ZERO_OR_MORE,
        default=["an", "unknown", "order"],
    ...
    add_parser.add_argument(
        "--name", "-n", type=str, help="orderer, if different from messenger, in quotes"
    )
    ...
    remove_parser = order_subparser.add_parser(
        cmd[2],
        help="remove a user's item from the current collective order, all items of a user or all users",
    )
    ...
    remove_parser.add_argument(
        "--all", "-a", action="store_true", help="all orders are removed"
    )
    ...
    cancel_parser = order_subparser.add_parser(
        cmd[5], help="cancel current collective order"
    )
    cancel_parser.set_defaults(func=cancel)

    print_parser = order_subparser.add_parser(
        cmd[7], help="display current collective order"
    )
    ...
    print_parser.add_argument(
        ...
        "-s",
        action="store_true",
        help="displays only the orders of the messenger",
    )

    reopen_parser = order_subparser.add_parser(
        cmd[13], help="reopen last order, if no current order"
    )
    reopen_parser.set_defaults(func=reopen)

    suggest_parser = order_subparser.add_parser(
        cmd[14], help="return the last 5 ordered item of the user, with pricing"
    )
    suggest_parser.set_defaults(func=suggest)

    reorder_parser = order_subparser.add_parser(
        cmd[18], help="reorder the last item of a user, from the last order"
    )
    reorder_parser.set_defaults(func=reorder)
    reorder_parser.add_argument("--name", "-n", type=str, nargs=argparse.ONE_OR_MORE)

    history_parser = order_subparser.add_parser(
        cmd[19], help="display history of the last [k] orders"
    )
    history_parser.set_defaults(func=history)
    history_parser.add_argument(
        "--k", "-k", type=int, default=5, help="number of orders to display"
    )

    edit_parser = order_subparser.add_parser(cmd[12], help="edit the price of an item")
    edit_parser.set_defaults(func=edit)
    edit_parser.add_argument(
        "order name", type=str, nargs=argparse.ONE_OR_MORE, help="name of order"
    )
    edit_parser.add_argument("new price", type=float, help="new price")
    edit_parser.add_argument(
        "--name", "-n", type=str, help="orderer, if different from messenger"
    )

    register_parser = user_subparser.add_parser(
        cmd[0],
    ...
    pay_parser = user_subparser.add_parser(
        cmd[17], help="transfer money from one user to another"
    )
    ...
    pay_parser.add_argument(
        "destination", type=str, nargs=argparse.ONE_OR_MORE, help="destination"
    )

    payout_parser = user_subparser.add_parser(
        cmd[8], help="get a suggestion for a potential payout"
    )
    ...
    if loglevel == logging.DEBUG:
        add_money_parser = user_subparser.add_parser(
            cmd[9], help="add initial balance to a user", prefix_chars="@"
        )
        add_money_parser.set_defaults(func=add_money)
        add_money_parser.add_argument(
            "name", nargs=argparse.ONE_OR_MORE, type=str, help="recipient"
        )
        add_money_parser.add_argument("balance", type=float, help="balance")

    balance_parser = user_subparser.add_parser(
        cmd[10], help="display the balance of all users"
    )
    balance_parser.set_defaults(func=balance)

    join_parser = user_subparser.add_parser(
        cmd[11], help="join system with matrix-address"
    )
    join_parser.add_argument(
        "--all",
        "-a",
        action="store_true",
        help="adds all current members of the room to the db",
    )
    ...
    init_parser.add_argument(
        "name", nargs=argparse.ONE_OR_MORE, type=str, help="name of user"
    )
    ...
    init_parser.add_argument(
        ...
        "-f",
        action="store_true",
        help="force initialization, even if balance is not zero",
    )

    deinit_parser = user_subparser.add_parser(
        cmd[16], help="deactivates user, if balance is zero"
    )
    deinit_parser.set_defaults(func=deinit)
    deinit_parser.add_argument(
        "--self", "-s", action="store_true", help="deinitializes messenger"
    )
    deinit_parser.add_argument(
        "name", nargs=argparse.ZERO_OR_MORE, type=str, help="name of user"
    )
    ...
    # eror handling for wrong input or help command
        ...
        if inp[0] in ["order", "user"]:
            if len(inp) > 1 and inp[1] in cmd and inp[0] == "order":
                possible_parsers = [
                    action
                    for action in order_parser._actions
                ...
                possible_parsers = [
                    action
                    for action in user_parser._actions
                    if isinstance(action, argparse._SubParsersAction)
                ]
                for parser_action in possible_parsers:
                    for choice, subparser in parser_action.choices.items():
